...
ROS_SOURCES = {
    'thebatx': (
        'rthebatx',
        'https://www.fangraphs.com/projections?type=rthebatx&stats=pit&pos=all&team=0&players=0&lg=all&pageitems=30&statgroup=standard&fantasypreset=dashboard',
    ),
    'oopsy': (
        'roopsydc',
        'https://www.fangraphs.com/projections?type=roopsydc&stats=pit&pos=all&team=0&players=0&lg=all&pageitems=30&statgroup=standard&fantasypreset=dashboard',
    ),
}
...
def main():
    ...
    for label, (fangraphs_type, page_url) in ROS_SOURCES.items():
        ...
        result = download_projections(
            label, fangraphs_type, 'pit', csv_path, page_url=page_url,
        )
        if result is None:
